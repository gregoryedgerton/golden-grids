...
composed exactly as the spiral places its neighbours. Framework-free: bind it
to scroll, a slider, or a clock. Depth 0 focuses the last (largest) square;
each whole step moves one square deeper, toward the eye.
...
Full API — per-tile transforms, the fade window, edge-culling, anchoring,
and solving which side the dial trails toward — is in
**[docs/spiral-dial.md](docs/spiral-dial.md)**.
...
A runnable example app lives in [`Examples/iOS`](Examples/iOS) — five screens:
a swipeable featured carousel, a sky gallery, a stats dashboard, a line-less
editorial layout, and a **Spiral** depth dial (drag or slide to dial through
ninety-one squares). See [`Examples/iOS/README.md`](Examples/iOS/README.md)
to build and run it, with screenshots.
...
A runnable example app lives in [`android/example`](android/example) — the
same five screens as the iOS example, rebuilt in Compose, including the
**Interactive Experiences** depth dial (91 squares, flick inertia, DUO/TRIO/QUAD
quick sets — full parity with the iOS screen). The renderer is verified
...
| --- | --- | --- | --- |
| 01 — Airbnb listing page | [live](https://gregoryedgerton.github.io/golden-grids-study-01-airbnb/) (pass one: structure and asset spec) | [repo](https://github.com/gregoryedgerton/golden-grids-study-01-airbnb) | Production viability: a listing's hierarchy is already golden; the twelve-column grid flattens it below the fold. |
