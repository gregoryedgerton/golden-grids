...
struct RootView: View {
    ...
    var body: some View {
        // Icons only, no labels. Five screens.
        TabView(selection: $tab) {
            ...
            SpiralView().tabItem { Image(systemName: "hurricane") }.tag(4)
        }
    }
}
...
private func initialTab() -> Int {
    if let raw = ProcessInfo.processInfo.environment["GG_TAB"], let i = Int(raw), (0...4).contains(i) {
        return i
    }
    return 0
}
